...
export default function ColorBends({
  ...
  // Use a stable reference to node for listeners and cleanup
  useEffect(() => {
    // Narrowing the ref value to a constant to ensure non-nullity and prevent never type issues in closures.
    ...
    const handleResize = () => {
      // Re-using the narrowed constant from outer scope.
      const w = currentElement.clientWidth || 1;
      const h = currentElement.clientHeight || 1;
    ...
    // Explicitly cast to HTMLDivElement to resolve the TypeScript 'never' error.
    (currentElement as HTMLDivElement).addEventListener('pointermove', handlePointerMove);
    ...
    return () => {
      ...
      // Using narrowed constant for final cleanup.
      currentElement.removeEventListener('pointermove', handlePointerMove);
      if (renderer.domElement && renderer.domElement.parentElement === currentElement) {
  ...
  return (
    <div ref={containerRef} className={`color-bends-container ${className}`} style={{
      position: 'relative',
      width: '100%',
      height: '100%',
      overflow: 'hidden',
